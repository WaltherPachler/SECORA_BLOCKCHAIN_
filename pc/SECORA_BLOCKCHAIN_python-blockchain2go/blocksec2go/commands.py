...
def create_key_label(reader, key_id: int, storage_size: int):
    ...
    if(r.sw == 0x6A88):
        ...
        raise Exception('0x6985 Key Label storage already available ')
    else:
        r.check()
...
def update_key_label(reader, key_id: int, label):
    ...
    len_data = len_label + 1 # 1 byte key id
    len_indicator = None
    if(len_data <= int(MAX_DATA_LEN / 2)):
        len_indicator = bytes([len_data])
    elif(len_data <= MAX_DATA_LEN):
        len_data += 2
        len_indicator = b'\x81' + bytes([len_data])
    elif(len_data <= MAX_LABEL_LEN):
        len_data += 2
        len_indicator = b'\x82' + len_data.to_bytes(2, 'big')
    else:
    ...
    rounds = int(len_data / MAX_DATA_LEN) + 1
    ...
    for i in range(0, rounds):
        ...
        logger.debug('data_to_send ' + str(hexlify(data_to_send)))
...
def get_status(reader):
    ...
    return int.from_bytes(r.resp, byteorder='big')
